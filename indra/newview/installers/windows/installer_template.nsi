...
#!define MUI_ICON   "%%SOURCE%%\installers\windows\lumen_icon.ico"
#!define MUI_UNICON "%%SOURCE%%\installers\windows\lumen_icon.ico"
!define MUI_ICON   "%%SOURCE%%\installers\windows\lumen_icon.ico"
!define MUI_UNICON "%%SOURCE%%\installers\windows\lumen_icon.ico"
# </FS:Ansariel>
...
!define LINDEN_KEY      "SOFTWARE\Lumen"
...
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "Publisher" "Catten Carter"
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "URLInfoAbout" "https://github.com/cattencarter/lumen"
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "URLUpdateInfo" "https://github.com/cattencarter/lumen/releases"
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "HelpLink" "https://github.com/cattencarter/lumen"
...
# Delete files in \Users\<User>\AppData\Roaming\Lumen_x64
# Remove all settings files but leave any other .txt files to preserve the chat logs
    RMDir /r "$2\AppData\Roaming\Lumen_x64\logs"
    RMDir /r "$2\AppData\Roaming\Lumen_x64\browser_profile"
    RMDir /r "$2\AppData\Roaming\Lumen_x64\user_settings"
    Delete  "$2\AppData\Roaming\Lumen_x64\*.xml"
    Delete  "$2\AppData\Roaming\Lumen_x64\*.bmp"
    Delete  "$2\AppData\Roaming\Lumen_x64\search_history.txt"
    Delete  "$2\AppData\Roaming\Lumen_x64\plugin_cookies.txt"
    Delete  "$2\AppData\Roaming\Lumen_x64\typed_locations.txt"
# Delete files in \Users\<User>\AppData\Local\Lumen
    ${If} ${ISOPENSIM} == "0"
        ${If} ${IS64BIT} == "0"
            RMDir /r "$2\AppData\Local\Lumen"				#Delete the Havok cache folder
        ${Else}
            RMDir /r "$2\AppData\Local\Lumen_x64"			#Delete the OpenSim cache folder
        ${EndIf}
    ${Else}
        ${If} ${IS64BIT} == "0"
            RMDir /r "$2\AppData\Local\Lumen"			#Delete the Havok cache folder
...
# Delete files in ProgramData\Lumen
Push $0
